...
AS
SELECT        dbo.transactions.TransactionID, dbo.transactions.UserID_From, dbo.transactions.UserID_To, dbo.transactions.Amount, dbo.transactions.Comment, dbo.transactions.DateStarted, dbo.transactions.ApprovedFrom, 
                         dbo.transactions.ApprovedTo, dbo.transactions.UserID, dbo.transactions.EventDate, user_to.FullName AS FullName_To, user_from.FullName AS FullName_From
FROM            dbo.transactions INNER JOIN
                         dbo.users AS user_from ON dbo.transactions.UserID_From = user_from.UserID INNER JOIN
                         dbo.users AS user_to ON dbo.transactions.UserID_To = user_to.UserID
    ...
    CASE
        ...
        WHEN Money_Transfer IS NOT NULL THEN
            CASE 
                ...
                WHEN t.TransactionID IS NOT NULL THEN CONCAT( 'Transfer:', t.FullName_From, ' > ', t.FullName_To )
...
--	WHILE @@FETCH_STATUS = 0
	...
	BEGIN
		...
		,@Impact_Net_Onetime		= @Impact_Net_Onetime		 + ISNULL( impact_onetime_w_risk, 0 )
		,@Impact_Net_Onetime_NoRisk	= @Impact_Net_Onetime_NoRisk + ISNULL( impact_onetime_flat, 0 )
...
ALTER VIEW [dbo].[View_Transactions]
AS
SELECT        dbo.transactions.TransactionID, dbo.transactions.UserID_From, dbo.transactions.UserID_To, dbo.transactions.Amount, dbo.transactions.Comment, dbo.transactions.DateStarted, dbo.transactions.ApprovedFrom, 
                         dbo.transactions.ApprovedTo, dbo.transactions.UserID, dbo.transactions.EventDate AS CreatedDate, dbo.users.FullName AS EventDate, users_1.FullName AS FullName_From
FROM            dbo.transactions INNER JOIN
                         dbo.users AS users_1 ON dbo.transactions.UserID_From = users_1.UserID INNER JOIN
                         dbo.users ON dbo.transactions.UserID_To = dbo.users.UserID
